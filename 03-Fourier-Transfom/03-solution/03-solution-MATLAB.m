...
# Exercise 2.1b: Amplitude spectra of a trumpet signal
data, samplingFreq = scipy.io.wavfile.read('../data/trumpet.wav')

nData = len(data)
nDft = nData
dftData = fft(data, nDft)

freqVector = samplingFreq * np.arange(nDft) / nDft
plt.plot(freqVector, np.abs(dftData))
plt.xlabel('Freq. [Hz]')
...
# a) compute convolution as a sum
ya = np.zeros(convLength)
for ii in range(convLength):
    for jj in range(nData):
        ya[ii] += hzp[jj] * xzp[(ii-jj) % convLength]
...
# c) compute the convolution in the frequency domain
yc = np.real(ifft(fft(h, convLength) * fft(x, convLength)))
